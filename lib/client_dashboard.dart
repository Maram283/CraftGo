import 'dart:ui';
import 'package:craftgo/product_details_page.dart';
...
class _ClientDashboardState extends State<ClientDashboard> {
  ...
  // Selected filter category index (0 = All)
  ...
  Future<void> _loadCategories() async {
    const iconMap = {
      ...
      'Woodwork': Icons.chair_outlined,
      'Calligraphy': Icons.edit_outlined,
      'Jewelry': Icons.watch_outlined,
      'Ceramics': Icons.local_cafe_outlined,
      'Painting': Icons.brush_outlined,
      'Macrame': Icons.auto_awesome,
    };
    try {
      ...
      setState(() {
        _categories = [
          {'id': null, 'titleAr': 'الكل', 'titleEn': 'All', 'icon': Icons.all_inclusive},
          ...raw.map((c) => {
            'id': c['id'],
            'titleEn': c['name'] ?? 'Category',
            'titleAr': c['name_ar'] ?? c['name'] ?? 'فئة',
            'icon': iconMap[c['name']] ?? Icons.category_outlined,
          }).toList(),
        ];
      });
    } catch (_) {
  ...
  Future<void> _loadProducts({String? categoryId}) async {
    ...
    try {
      ...
      setState(() {
        _products = raw.map((p) => {
          ...
          'price': (p['price'] != null)
              ...
              : 0.0,
          'rating': (p['artisan_rating'] != null)
              ? p['artisan_rating'].toString()
              : '—',
          ...
          'artisan_rating': p['artisan_rating'],
        ...
        final seen = <String>{};
        _artisans = _products
            .where((p) => p['artisan_id'] != null && seen.add(p['artisan_id'] as String))
            .take(5)
            .map((p) => {
          'id': p['artisan_id'],
          'nameAr': p['artisan_name'] ?? '',
          'nameEn': p['artisan_name'] ?? '',
          'craftAr': p['category_name_ar'] ?? p['category_name'] ?? '',
          'craftEn': p['category_name'] ?? '',
          'rating': p['artisan_rating']?.toString() ?? '—',
          'jobs': '—',
          'image': p['artisan_avatar'] ?? '',
        }).toList();
      ...
      setState(() {
        _isLoadingProducts = false;
        _errorMessage = e is ApiException ? e.message : 'Connection error';
      });
    }
  }
  ...
  Color get cardBorderColor =>
      isDarkMode ? Colors.white.withValues(alpha: 0.12) : Colors.black.withValues(alpha: 0.08);
  ...
  List<Map<String, dynamic>> get categories => _categories.isNotEmpty
      ? _categories
      : [{"id": null, "name": "All", "name_ar": "الكل", "icon": null}];

  List<Map<String, dynamic>> get craftsmen => _artisans;

  List<Map<String, dynamic>> get products => _products;

  List<Map<String, dynamic>> get filteredProducts {
    // Filter by selected tab (popular/new/recommended)
    // For now, all products show in all tabs until tags are added to DB
    return _products;
  }
              ...
              child: Column(
                ...
                children: [
                            ...
                            Container(
                              ...
                              decoration: BoxDecoration(
                                color: activeFilterCount > 0
                                    ? const Color(0xFFD4A017)
                                    : (isDarkMode ? Colors.white.withValues(alpha: 0.04) : Colors.black.withValues(alpha: 0.02)),
                                borderRadius: BorderRadius.circular(15),
                                border: Border.all(
                  ...
                  if (_isLoadingProducts)
                    const SizedBox(
                      height: 255,
                      child: Center(child: CircularProgressIndicator(color: Color(0xFFD4A017))),
                    )
